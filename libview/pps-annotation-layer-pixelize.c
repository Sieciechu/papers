 ...
 *
 * Pixelized regions are persisted by creating a stamp annotation whose
 * appearance is the pixelized image.  Poppler embeds the image in the PDF
 * so it survives save/reload.  The stamp area matches the dragged rectangle
 * exactly in Papers doc coordinates (x/y in screen-space units / scale).
 *
...
/*
 * create_pixelized_surface:
 *
 * Renders the pixelated version of the page region [wx1,wy1]×[wx2,wy2]
 * (widget-space coordinates) into a newly allocated cairo_image_surface_t.
 * The surface is (clip_w × clip_h) pixels in size and contains the
 * CAIRO_FILTER_NEAREST upscale of a bilinearly-downsampled thumbnail.
 *
 * Returns: (transfer full) a new cairo_surface_t, or NULL on failure.
 *          Caller must cairo_surface_destroy() it.
 */
static cairo_surface_t *
create_pixelized_surface (GdkTexture *page_tex,
                          int         widget_w,
                          int         widget_h,
                          gdouble     wx1,
                          gdouble     wy1,
                          ...
                          gdouble     wy2)
{
	...
	/* ── Pass 1: downsample clip → tiny pix_surf ───────────────────────── */
	...
	/* ── Pass 2: scale back up via NEAREST → output surface ────────────── */
	int out_w = (int) ceil (clip_w);
	int out_h = (int) ceil (clip_h);
	cairo_surface_t *out_surf = cairo_image_surface_create (
	    CAIRO_FORMAT_ARGB32, out_w, out_h);
	cairo_t *out_cr = cairo_create (out_surf);

	cairo_pattern_t *pat = cairo_pattern_create_for_surface (pix_surf);
	cairo_matrix_t mat;
	/* Output surface origin = (0,0); pix_surf pixel i covers [i*S, (i+1)*S].
	 * Pattern matrix: out_coord → pix_coord = out_coord / PIXEL_SIZE. */
	cairo_matrix_init_scale (&mat, 1.0 / PIXEL_SIZE, 1.0 / PIXEL_SIZE);
	cairo_pattern_set_matrix (pat, &mat);
	cairo_pattern_set_filter (pat, CAIRO_FILTER_NEAREST);
	cairo_set_source (out_cr, pat);
	cairo_paint (out_cr);
             ...
             PpsAnnotationLayerPixelize *self)
{
	...
	/* ── 1. Create stamp annotation so the region survives save/reload ── */
	GdkTexture *page_tex = pps_pixbuf_cache_get_texture (priv->pixbuf_cache, page);
	if (page_tex) {
		cairo_surface_t *stamp_surf = create_pixelized_surface (
		    page_tex, widget_w, widget_h, wx1, wy1, wx2, wy2);

		if (stamp_surf) {
			/* Papers doc-coord system: Y-down (same as screen), divided by scale.
			 * x1/y1 = top-left, x2/y2 = bottom-right. */
			gdouble wxMin = MIN (wx1, wx2), wyMin = MIN (wy1, wy2);
			gdouble wxMax = MAX (wx1, wx2), wyMax = MAX (wy1, wy2);
			PpsPoint doc_start = { wxMin / scale, wyMin / scale };
			PpsPoint doc_end   = { wxMax / scale, wyMax / scale };

			static const GdkRGBA transparent = { 0, 0, 0, 0 };
			/* The Poppler backend (pdf_document_annotations_add_annotation)
			 * automatically tags stamp annotations via poppler_annot_set_contents
			 * so that pdf_document_save can identify them and rasterize the page
			 * to truly destroy the underlying text. */
			pps_annotations_context_add_annotation_sync (
			    GET_ANNOT_CONTEXT (self),
			    page,
			...
			cairo_surface_destroy (stamp_surf);
		}
	}

	/* ── 2. Keep an in-memory copy for live rendering during the session ─ */
	gdouble page_height = 0.0;
	PpsDocument *doc = pps_document_model_get_document (GET_DOC_MODEL (self));
